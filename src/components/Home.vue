<template>
  <div
    id="loading"
    wire:loading
    class="fixed top-0 left-0 right-0 overflow-hidden bottom-0 w-full h-screen z-[999] overflow-hidden bg-bleck flex flex-col items-center justify-center"
  >
    <div
    ...
    <div class="flex justify-between items-center mx-12">
      ...
      <div class="button flex items-center">
        <div class="flex bg-bleck-1 mr-2 text-white rounded-xl">
          <input
            type="radio"
            @click="FahrenToCelsius()"
            id="celsius"
            checked
            name="check-substitution-2"
          />
          <label class="p-4" for="celsius">°C</label>
        </div>
        <div class="flex bg-bleck-1 text-white rounded-xl">
          <input
            type="radio"
            @click="CelsiusToFahren()"
            id="Fahrenheit"
            name="check-substitution-2"
          />
          <label class="p-4" for="Fahrenheit">°F</label>
        </div>
      </div>
    </div>
  </nav>
  <div id="content" class="bg-bleck pt-20 w-full h-full max-lg:flex-wrap flex text-white">
    <div class="flex flex-col min-w-[25%] max-lg:w-full w-1/3 p-4 w-2/6 mx-8">
    ...
    <div class="flex flex-col m-4 mr-8 w-full">
      <div class="flex flex-col bg-bleck-1 rounded-xl">
        ...
        <div class="flex p-4 max-lg:flex-wrap gap-2">
          <div class="flex flex-col w-3/6 max-lg:w-full bg-bleck rounded-xl p-4">
            <h1 class="p-4 txt-blk">Air Quality Index</h1>
            <div
              class="grid grid-cols-4 max-xl:grid-cols-2 max-sm:grid-cols-1 justify-items-center items-center justify-around pb-6 p-4"
            >
          ...
          <div class="flex flex-col w-3/6 max-lg:w-full bg-bleck rounded-xl p-4">
            <h1 class="txt-blk p-4 max-lg:text-center">Sunrise & Sunset</h1>
            <div class="flex items-center justify-around p-4 max-lg:flex-wrap gap-2 h-full">
        ...
        <div class="grid-cols-4 max-lg:grid-cols-2 max-sm:grid-cols-1 grid grid m-6 mr-5 gap-4">
      ...
      <ul
        id="tempPerH"
        class="flex gap-3 flex-[1_1_100%] max-lg:justify-center flex-wrap min-w-[110px]"
      ></ul>
    </div>
  </div>
</template>
    ...
    const StartUp = () => {
      ...
      https: $.getJSON('https://api.ipify.org?format=json', function (data) {
        ...
        fetch(
          ...
          .then((data) => {
            console.log(data)
            $('#temp').text(Math.round(data.current.feelslike_c) + '℃')
            ...
            for (let i = 1; i < 10; i++) {
              ...
              $('#forecast').append(
                '<div class="justify-between text-xl my-2 gap-4 flex items-center"><img src=' +
                  forecastTempIcon +
                  ' class="w-9" alt="" /><span id="tempInC' +
                  i +
                  '" class="">' +
                  forecastTemp +
                  '℃</span><span class="">' +
